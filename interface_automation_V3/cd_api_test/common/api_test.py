...
class TestCase:
    ...
    def checkFailCase(self, project):
        ...
        my_db = MysqlDb()
        # 获取接口域名
        sql1 = "select count(*) as case_num from {0}_testcases where project ='{1}' and pass!='Pass' and run='yes'".format(
            project, project)
        cases_num = my_db.query(sql1)
        results.append(cases_num)
        sql2 = "select * from {0}_testcases where project ='{0}' and pass!='Pass' and run='yes'".format(project,
                                                                                                        project)
    ...
    def runCase(self, project, case, api_host_obj):
        ...
        if case["pre_case_id"] > -1:
            ...
            if bool(pre_assert_msg['is_pass']):
            ...
            for pre_field in pre_fields:
                if pre_field['scope'] == 'header':
                    ...
                    for header_key in headers:
                        ...
                        if field_name in str(headers[header_key]):
                            field_value = pre_response[field_name]
                            headers[header_key] = str(headers[header_key]).replace('$' + field_name + '$', field_value)
    ...
    def assertResponse(self, case, response):
        ...
        if assert_type == 'code':
            ...
            if str(expect_result) == str(response_code):
                is_pass = True
        ...
        msg = "module:{0},title:{1},assert_type:{2}".format(case['module'], case['title'], assert_type)

        # message = "module:{0},title:{1},assert_type:{2},message:{3}".format(case['module'], case['title'], assert_type,
        #                                                                     response['message'])
        # 拼装信息
